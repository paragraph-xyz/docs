...
function Header({ navigation }) {
  ...
  return (
    <header
      className={clsx(
        'sticky top-0 z-50 flex flex-wrap items-center justify-between bg-background px-4 py-5 shadow-md shadow-foreground/5 transition duration-500 sm:px-6 lg:px-8',
        {
          'backdrop-blur [@supports(backdrop-filter:blur(0))]:bg-background/75':
            isScrolled,
          'bg-background/90': !isScrolled,
        }
      )}
    >
    ...
    <>
      ...
      <div className="relative mx-auto flex max-w-8xl justify-center sm:px-2 lg:px-8 xl:px-12">
        <div className="hidden lg:relative lg:block lg:flex-none">
          <div className="absolute inset-y-0 right-0 w-[50vw] bg-muted" />
          <div className="sticky top-[4.5rem] -ml-0.5 h-[calc(100vh-4.5rem)] overflow-y-auto py-16 pl-0.5">
            <div className="absolute top-16 bottom-0 right-0 h-12 w-px bg-gradient-to-t from-border" />
            <div className="absolute top-28 bottom-0 right-0 w-px bg-border" />
            <Navigation
          ...
          <article>
            {(title || section) && (
              <header className="mb-9 space-y-1">
                {section && (
                  <p className="font-display text-sm font-medium text-link-underline">
                    {section.title}
                  </p>
                )}
                {title && (
              ...
              <div>
                ...
                <dd className="mt-1">
                  <Link href={previousPage.href}>
                    <a className="text-base font-semibold text-muted-foreground hover:text-foreground">
                  ...
                  {tableOfContents.map((section) => (
                    <li key={section.id}>
                      ...
                      {section.children.length > 0 && (
                        <ul className="mt-2 space-y-3 pl-5 text-muted-foreground">
